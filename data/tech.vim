...
call s:def('oil_refining',        1890, 'ind', '石油精製',             'Oil Refining',            2000, ['oil_drilling'], {'out_all': 1.03, 'navy': 1.05})
...
call s:def('clipper_ships',       1836, 'infra', '快速帆船',           'Clipper Ships',            200, [], {'trade': 1.1, 'navy': 1.1})
...
call s:def('steamships',          1850, 'infra', '蒸気船',             'Steamships',               500, ['steam_engine'], {'trade': 1.2, 'navy': 1.15})
...
call s:def('compound_engine',     1870, 'infra', '複式蒸気機関',       'Compound Engine',          900, ['steamships'], {'trade': 1.1, 'navy': 1.1})
call s:def('transatlantic_cable', 1870, 'infra', '大洋横断ケーブル',   'Transoceanic Cables',     1000, ['telegraph'], {'trade': 1.15, 'research': 1.05})
call s:def('steel_hulls',         1870, 'infra', '鋼鉄船体',           'Steel Hulls',             1100, ['bessemer_process'], {'trade': 1.15, 'mil': 1.05, 'navy': 1.15})
...
call s:def('coastal_fortifications', 1850, 'war', '沿岸要塞',          'Coastal Fortifications',   400, [], {'mil': 1.05, 'navy': 1.05})
...
call s:def('ironclads',           1870, 'war', '装甲艦',               'Ironclads',               1000, ['steam_engine'], {'mil': 1.1, 'trade': 1.05, 'navy': 1.2})
call s:def('repeating_rifles',    1870, 'war', '連発銃',               'Repeating Rifles',        1000, ['breech_loaders'], {'mil': 1.1})
call s:def('torpedoes',           1890, 'war', '魚雷',                 'Torpedoes',               1700, [], {'mil': 1.05, 'navy': 1.1})
...
call s:def('naval_gunnery',       1910, 'war', '射撃管制',             'Naval Fire Control',      2900, ['modern_artillery'], {'mil': 1.1, 'navy': 1.15})
call s:def('submarines',          1910, 'war', '潜水艦',               'Submarines',              3000, ['oil_refining'], {'mil': 1.1, 'navy': 1.15})
call s:def('dreadnoughts',        1910, 'war', '弩級戦艦',             'Dreadnoughts',            3200, ['steel_hulls'], {'mil': 1.15, 'navy': 1.25})
...
call s:def('aircraft_carriers',   1930, 'war', '航空母艦',             'Aircraft Carriers',       5000, ['military_aviation', 'dreadnoughts'], {'mil': 1.15, 'navy': 1.25})
call s:def('radar',               1930, 'war', 'レーダー',             'Radar',                   5200, ['radio'], {'mil': 1.15, 'navy': 1.1})
...
call s:def('republicanism',       1836, 'idea', '共和主義',             'Republicanism',            400, ['liberal_thought'], {'research': 1.03})
...
call s:def('pax_britannica',      1850, 'nat', 'パクス・ブリタニカ',   'Pax Britannica',           600, [], {'trade': 1.25, 'mil': 1.05, 'navy': 1.1}, ['GBR'])
...
call s:def('two_power_standard',  1890, 'nat', '二国標準主義',         'Two-Power Standard',      1900, [], {'mil': 1.15, 'navy': 1.3}, ['GBR'])

...
開発計画は [PLANNING.md](PLANNING.md) を参照。現在は **M2(プレイアブル)** まで実装済み:
34 の国民市場で毎週、建物の生産・Pop の需要・需給価格・賃金・雇用調整が回る。
建設画面(`gc`)で自国の州に建物を建て、予算画面(`gb`)で税率を調整して
「建てる → 儲かる → また建てる」のループが遊べる。建設は木材・工具・鋼鉄を
市場価格で消費するため、建てるほど資材価格が上がる。
...
| `h j k l` | マップ上で州を選択(建設画面では `j k` がメニュー選択) |
| `Enter` | マップ: 州の詳細 / 建設画面: キューへ追加 |
| `gm` `gb` `gc` | 市場 / 予算 / 建設 画面 |
| `x` | 建設画面: キュー末尾を取消 |
| `+` `-` | 予算画面: 税率を変更 |
| `gt` `gp` | 技術 / Pop 画面(M3 で実装) |

...
let s:T = {
      ...
      \ 'mk_cols':  ['%9s %9s %9s %9s %9s  %s', '%9s %9s %9s %9s %9s  %s'],
      \ 'mk_col_price': ['価格', 'Price'],
      \ 'mk_col_base':  ['基準', 'Base'],
      \ 'mk_col_world': ['世界', 'World'],
      \ 'mk_col_buy':   ['買い注文', 'Buy'],
      ...
      \ 'mk_exp':   ['→輸出 %.0f', '→exp %.0f'],
      \ 'mk_imp':   ['←輸入 %.0f', '←imp %.0f'],
      \ 'mk_tariff': ['  先週の関税収入 £%s(交易量に比例。戦争中は交易が半減する)',
      \              '  Last week''s tariff income £%s (scales with trade; halved while at war)'],
      \ 'mk_note':  ['  ※価格は需給で毎週変動する(基準価格の ±75% でクランプ)。割安な財は輸出され、割高な財は輸入される。',
      \             '  Prices move weekly with supply and demand (clamped to ±75% of base). Cheap goods are exported, dear goods imported.'],
      \
      \ 'bg_title':    ['  ━━ 予算: %s ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
      \                '  ━━ Budget: %s ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'],
      ...
      \ 'bg_row_tariff':   ['    関税収入      +£%s', '    Tariffs         +£%s'],
      ...
      \ 'tc_cur':    ['  研究済み %d/%d ┃ 研究力 %.1frp/週 ┃ 研究中: %s',
      \              '  Researched %d/%d ┃ research %.1frp/wk ┃ current: %s'],
      \ 'tc_req':    [' 要:', ' req:'],
      ...
      \ 'pl_happy': ['(満足)', '(pleased)'],
      \ 'pl_angry': ['(不満)', '(angry)'],
      \ 'pl_movs':  ['  ── 社会運動 ──', '  ── Social movements ──'],
      \ 'pl_mov_row': ['    %s 支持 %3.0f%%  要求: %s', '    %s support %3.0f%%  demands: %s'],
      \ 'pl_mov_none': ['  (活動中の社会運動はない)', '  (no active movements)'],
      \ 'pl_laws':  ['  ── 法律 ──', '  ── Laws ──'],
      ...
      \ 'err_law_tech':    ['思想「%s」の研究が必要です', 'Requires researching the idea "%s"'],
      ...
      \ 'log_movement':    ['【社会運動】%s が起こった', '[Movement] %s has begun'],

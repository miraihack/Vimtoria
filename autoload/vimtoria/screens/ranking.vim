...
function! vimtoria#screens#ranking#render(st) abort
  ...
  call add(l:lines, '   順位 ' . vimtoria#ui#pad('国', 24)
        \ . printf('%12s %8s %6s %14s', '週間GDP', '生活水準', '技術', '国庫'))
  call add(l:lines, '  ' . repeat('─', 74))
  let l:rank = 1
  for l:row in l:rows
    let [l:cid, l:gdp, l:sol, l:ntech, l:gold] = l:row
    let l:me = l:cid ==# a:st.country
    if l:rank <= 20 || l:me
      call add(l:lines, printf('  %s%3d. %s%11s£ %8.2f %5d %13s£',
            \ l:me ? '→' : ' ', l:rank,
            \ vimtoria#ui#pad(l:map.countries[l:cid].name . (l:me ? '(自国)' : ''), 24),
            \ vimtoria#ui#fmt_num(float2nr(l:gdp)), l:sol, l:ntech,
